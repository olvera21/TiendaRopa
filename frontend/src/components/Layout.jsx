...
import {
  ...
  UserCog, UserCircle, LogOut, Store, Calculator, Menu, X,
} from 'lucide-react';
...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen flex bg-parchment-100">
      ...
      <aside
        className={`fixed md:static inset-y-0 left-0 z-40 w-64 bg-ink-900 text-parchment-100 flex flex-col shrink-0
          transform transition-transform duration-200 ease-out
          ${mobileOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}
      >
        <div className="flex items-center justify-between gap-2 px-5 py-5 border-b border-white/10">
          <div className="flex items-center gap-2">
            <div className="w-9 h-9 rounded-lg bg-copper-500 flex items-center justify-center shrink-0">
          ...
          <button onClick={() => setMobileOpen(false)} className="md:hidden p-1 text-parchment-100/60 hover:text-parchment-100">
            <X size={20} />
          </button>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="md:hidden flex items-center gap-3 px-4 py-3 bg-ink-900 text-parchment-100 shrink-0 sticky top-0 z-20">
          <button onClick={() => setMobileOpen(true)} className="p-1 -ml-1 text-parchment-100/80">
            <Menu size={22} />
          </button>
          <div className="w-7 h-7 rounded-lg bg-copper-500 flex items-center justify-center shrink-0">
            <Store size={14} className="text-white" />
          </div>
          <p className="font-display font-semibold text-sm">Punto Family</p>
        </div>
